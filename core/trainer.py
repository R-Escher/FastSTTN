...
class Trainer():
    ...
    def load_latest(self):
        ''' Loads latest .pth files. If they don't exist, try to load .pth by iteration number in save.ckpt file. '''
        model_path = self.config['save_dir']
    ...
    def save_best(self):
        if self.config['global_rank'] == 0:
            gen_path = os.path.join( self.config['save_dir'], 'gen_best.pth' )
            dis_path = os.path.join( self.config['save_dir'], 'dis_best.pth' )
            opt_path = os.path.join( self.config['save_dir'], 'opt_best.pth' )
            if isinstance(self.netG, torch.nn.DataParallel) or isinstance(self.netG, DDP):
                netG = self.netG.module
                netD = self.netD.module
            else:
    ...
    def _train_epoch(self, pbar):
        ...
        for frames, masks in self.train_loader:
            ...
            # save iteration with best hole loss
            if ( (self.iteration % 100 == 0) and (hole_loss.item() < self.lowest_hole_loss)):
                self.lowest_hole_loss = hole_loss.item()
                print(f"saving iteration {self.iteration} with {hole_loss.item():.3f}")
                self.save_best()
