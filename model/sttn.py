...
from model.reversible import ReversibleSequence
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        for _ in range(stack_num):
            blocks.append(TransformerBlock(patchsize, hidden=channel//2))
        self.transformer = ReversibleSequence(nn.ModuleList(blocks))
    ...
    def forward(self, masked_frames, masks):
        ...
        # the encoder expands the channel dim from 3 to 256
        # and divides the width and height by 4, because of strides (2*1*2*1) = 4.
        # i.e. [3, 240, 432] --> [256, 60, 108]
        _, c, h, w = enc_feat.size()
        masks = F.interpolate(masks, scale_factor=1.0/4, recompute_scale_factor=True)

        # runs through transformer
        enc_feat = self.transformer(enc_feat, kwargs={'batch': b})
        # [bt, c, h, w]
    ...
    def infer(self, feat, masks):
        ...
        masks = F.interpolate(masks, scale_factor=1.0/4, recompute_scale_factor=True)
        feat = self.transformer(kwargs={'mask': masks, 'batch': 1})
        # [t, c, h, w] 
        return feat
...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, m, isBackward=False):
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, patchsize, d_model):
        ...
        # calculates query, key and value vectors from encoded features
    ...
    def forward(self, x, kwargs):
        ...
        self.b = kwargs['batch']
        self.m = kwargs['mask']
        self.isBackward = kwargs['isBackward']
        ...
        for (width, height), query, key, value in zip(self.patchsize,
            ...
            # m -> b, t, 1 (channel), h, w
            mm = self.m.view(self.b, t, 1, out_h, height, out_w, width)
            mm = mm.permute(0, 1, 3, 5, 2, 4, 6).contiguous().view(self.b,  t*out_h*out_w, height*width)
            mm = (mm.mean(-1) > 0.5).unsqueeze(1).repeat(1, t*out_h*out_w, 1)

            query = query.view(self.b, t, d_k, out_h, height, out_w, width)
            query = query.permute(0, 1, 3, 5, 2, 4, 6).contiguous().view(self.b,  t*out_h*out_w, d_k*height*width)
            key = key.view(self.b, t, d_k, out_h, height, out_w, width)
            key = key.permute(0, 1, 3, 5, 2, 4, 6).contiguous().view(self.b,  t*out_h*out_w, d_k*height*width)
            value = value.view(self.b, t, d_k, out_h, height, out_w, width)
            value = value.permute(0, 1, 3, 5, 2, 4, 6).contiguous().view(self.b,  t*out_h*out_w, d_k*height*width)

            y, _ = self.attention(query, key, value, mm, self.isBackward)

            y = y.view(self.b, t, out_h, out_w, d_k, height, width)
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, feat, mask, b, c):
        x, m, b, c = feat, mask, b, c 
